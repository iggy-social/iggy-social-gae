<template>
  <Head>
    <Title>Video NFTs | {{ $config.public.projectMetadataTitle }}</Title>
    <Meta property="og:title" :content="'Video NFTs | '+$config.public.projectMetadataTitle" />
...
export default {
  ...
  methods: {
    async fetchNfts(clear="false") {
      ...
      // Fetch NFTs
      try {
        let url = `/api/endpoint/read/nft-list-video?limit=${this.limit}`;
