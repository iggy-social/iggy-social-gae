<template>
  ...
  <div 
    class="modal fade" 
    id="addMissingNftModal" 
    aria-labelledby="addMissingNftModalLabel"
  >
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <p>
            So help us out and add the address of the NFT (launched via this dApp) that
            <span v-if="isCurrentUsersProfile">you hold</span> 
            <span v-if="!isCurrentUsersProfile">this user holds</span> 
            here:
          </p>
...
export default {
  ...
  methods: {
    async addNftToApi() {
      ...
      try {
        let url = `/api/endpoint/write/user-nfts-add?nft_address=${this.nftAddressToAdd}&user_address=${this.uAddress}`;
        await axios.get(url);
    ...
    async fetchNfts(clear="false") {
      ...
      // Fetch NFTs
      try {
        let url = `/api/endpoint/read/user-nfts-minted?user_address=${this.uAddress}&limit=${this.limit}`;
    ...
    handleCloseClick() {
      ...
      const closeButton = document.getElementById('closeAddMissingNftModal')
      if (closeButton) {
        closeButton.blur()
      }
    },
